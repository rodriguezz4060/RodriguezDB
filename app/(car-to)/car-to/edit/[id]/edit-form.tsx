...
import { useCarToForm } from '@/shared/hooks/'
...
export function EditForm({ carTo }: Props) {
  ...
  const form = useForm<TFormEditClientCarToSchema>({
    ...
    defaultValues: {
      ...
      engineOil: carTo.engineOil ?? '',
      engineOilVolume: carTo.engineOilVolume ?? '',
      engineOilPartNumber: carTo.engineOilPartNumber ?? '',

      automaticTransmissionOil: carTo.automaticTransmissionOil ?? '',
      automaticTransmissionOilVolume1: carTo.automaticTransmissionOilVolume1 ?? '',
      automaticTransmissionOilPartNumber: carTo.automaticTransmissionOilPartNumber ?? '',

      mechanicTransmissionOil: carTo.mechanicTransmissionOil ?? '',
      mechanicTransmissionOilVolume: carTo.mechanicTransmissionOilVolume ?? '',
      mechanicTransmissionOilPartNumber: carTo.mechanicTransmissionOilPartNumber ?? '',

      transferCaseOil: carTo.transferCaseOil ?? '',
      transferCaseOilVolume: carTo.transferCaseOilVolume ?? '',
      transferCaseOilPartNumber: carTo.transferCaseOilPartNumber ?? '',

      frontAxleGearboxOil: carTo.frontAxleGearboxOil ?? '',
      frontAxleGearboxOilVolume: carTo.frontAxleGearboxOilVolume ?? '',
      frontAxleGearboxOilPartNumber: carTo.frontAxleGearboxOilPartNumber ?? '',

      rearAxleGearboxOil: carTo.rearAxleGearboxOil ?? '',
      rearAxleGearboxOilVolume: carTo.rearAxleGearboxOilVolume ?? '',
      rearAxleGearboxOilPartNumber: carTo.rearAxleGearboxOilPartNumber ?? '',

      antifreeze: carTo.antifreeze ?? '',
      antifreezeVolume: carTo.antifreezeVolume ?? '',
      antifreezePartNumber: carTo.antifreezePartNumber ?? '',

      steeringFluid: carTo.steeringFluid ?? '',
      steeringFluidVolume: carTo.steeringFluidVolume ?? '',
      steeringFluidPartNumber: carTo.steeringFluidPartNumber ?? '',
    },
  })
  ...
  const { onSubmit, isSubmitting } = useCarToForm()
